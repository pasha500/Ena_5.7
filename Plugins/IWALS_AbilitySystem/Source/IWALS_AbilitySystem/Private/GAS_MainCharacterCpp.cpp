...
void AGAS_MainCharacterCpp::EnsureCompassWidget()
{
	if (CompassWidgetInstance) return;
	if (!IsLocallyControlled()) return;

	if (!CompassWidgetClass)
	{
		UClass* FallbackCompassClass = LoadClass<UUserWidget>(nullptr, TEXT("/Game/Game/AILevel/Blueprints/Interfaces/WBP_RaidCompass.WBP_RaidCompass_C"));
		if (!FallbackCompassClass)
		{
			FallbackCompassClass = LoadClass<UUserWidget>(nullptr, TEXT("/Game/CompassAndMarkers/Sources/Blueprints/Compass/WBP_Base_CompassBar.WBP_Base_CompassBar_C"));
		}
		if (FallbackCompassClass)
		{
			CompassWidgetClass = FallbackCompassClass;
		}
	}
	...
	CompassWidgetInstance = CreateWidget<UUserWidget>(PC, CompassWidgetClass);
	if (CompassWidgetInstance)
	{
		CompassWidgetInstance->AddToViewport(10);
		UE_LOG(LogTemp, Warning, TEXT("[GAS_MainCharacterCpp] Compass widget created: %s"), *GetNameSafe(CompassWidgetClass));
	}
}
